...
function TripCard({ trip, onDelete }) {  //it gets trip details and props
  ...
  const handleCardClick = () => {
    navigate(`/trip/${trip.id}`);   //chnages th erouute 
  };

  // Delete without triggering card click
  ...
  return (
    <div 
      ...
      className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-4 cursor-pointer hover:shadow-lg transition"
    >

      {/* photos carousel */}
      {trip.photo_urls && trip.photo_urls.length > 0 && (
        <div className="w-full h-36 bg-green-100 overflow-x-auto flex"> 
      ...
      <div className="p-5">
        ...
        <div className="flex justify-between items-start mb-3">
          <div className="flex-1">
            <h3 
              className="font-bold text-lg text-gray-800 hover:text-blue-800 transition cursor-pointer"
              onClick={handleCardClick}
            >
              {trip.title}
            </h3>
        ...
        {/*This part is just brings the data from trip variable and displays it here*/}   
        {trip.description && (
        ...
        {trip.challenges && trip.challenges.length > 0 && (
          <div className="text-xs">
            <p className="text-gray-600 font-semibold mb-1">Challenges</p>
...
//If someone imports from this file, give them TripCard by default.
